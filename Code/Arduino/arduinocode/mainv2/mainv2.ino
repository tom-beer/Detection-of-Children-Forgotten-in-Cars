...
#define   SMP_RATE          500UL  // for FFT_size = 2048 version
...
void histogram_check(short *cnt, bool *state) {
  ...
  if ((*cnt == 0)&&(*state == true)) *state=false;
  else if ((*cnt == HISTLVL) && (*state == false)) *state=true;
}

int check_serial_port() {
  Serial.println("check serial port");
  int print_inp = 0;
